...
// =============================================
// 8. FRONTEND COMPATIBILITY ROUTES (ADDED)
// =============================================

// Create shop (for frontend) - uses phone + fullName
...
// Login (for frontend) - uses shop_name + password
app.post('/api/login', (req, res) => {
  const { shopName, password } = req.body;
  
  if (!shopName || !password) {
    return res.status(400).json({ success: false, error: 'Shop name and password required' });
  }

  db.get(`SELECT * FROM users WHERE shop_name = ?`, [shopName], async (err, user) => {
    if (err) {
      console.error('Login error:', err);
      return res.status(500).json({ success: false, error: err.message });
    }
    if (!user) {
      return res.status(401).json({ success: false, error: 'Shop not found' });
    }
